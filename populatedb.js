...
//var async =  require('./node_modules/async/index');
...
async function authorCreate(first_name, family_name, d_birth, d_death, cb) {
  ...
  try{
      ...
      console.log(`>>>>>>>>>> New Author: ${author.full_name} id: ${author.id}`);
      authors.push(author);
      cb(null, author);
  } catch (err) {
      cb(err, null);
...
async function genreCreate(name, cb) {
  try{
    ...
    console.log(`>>>>>>>>>> New Genre: ${genre.name} id: ${genre.id}`);
    genres.push(genre);
    cb(null, genre);
  } catch (err) {
      cb(err, null);
...
async function bookCreate(title, summary, isbn, author, genre, cb) {
  let bookdetail = { 
    title: title,
    summary: summary,
    authorId: author.id, // references authors model (must be = 1)
    isbn: isbn
  } 
...
async function bookInstanceCreate(book, imprint, due_back, status, cb) {
  let bookinstancedetail = { 
    bookId: book.id, // references books model (must be = 1)
    imprint: imprint
  }    
    ...
    async.parallel([
        function(callback) {
          bookCreate(
            ...
            'from the University at a younger age than most people are allowed in. I tread paths by moonlight' + 
            'that others fear to speak of during day. I have talked to Gods, loved women, and written songs that make the minstrels weep.', 
        ...
        function(callback) {
          bookCreate(
            ...
            '9780756411336', authors[0], [genres[0]], callback);
        },
        function(callback) {
          bookCreate(
            "Apes and Angels", 
            "Humankind headed out to the stars not for conquest, nor exploration, nor even for curiosity. Humans went to the stars in a desperate crusade to save intelligent life wherever they found it. A wave of death is spreading through the Milky Way galaxy, an expanding sphere of lethal gamma ...", 
            '9780765379528', authors[1], [genres[1]], callback);
        },
        function(callback) {
          bookCreate(
            "Death Wave",
...
function createBookInstances(cb) {
    async.parallel([
        function(callback) {
          bookInstanceCreate(books[0], 'London Gollancz, 2014.', '2022-02-14', 'Available', callback)
        },
        function(callback) {
